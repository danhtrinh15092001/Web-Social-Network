<?php
    ...
    // <div role="button" tabindex="0" data-focusable="true" class="getValue h-ment">'.$_POST['hashtag'].'</div>
    // </li>';
    ...
    if (is_post_request()){
        if (isset($_POST['hashtag']) && !empty($_POST['hashtag'])){
            $hashtag_1=FormSanitizer::formSanitizerString($_POST['hashtag']);
            $mention=FormSanitizer::formSanitizerString($_POST['hashtag']);
            if (substr($hashtag_1,0,1)==='#'){
                $trend=str_replace('#','',$hashtag_1);
                $trends=$loadFromTweet->getTrendByHash($trend);
                foreach ($trends as $hashtag_1)
                {
                    echo '<li role="option" aria-selected="true">
                    <div role="button" tabindex="0" data-focusable="true" class="getValue h-ment">#'.$hashtag_1->hashtag.'</div>
                    </li>';  
                }
            }
                        ...
                        <div class="h-ment__container">
                            ...
                            <div class="container__user-info">
                                <div class="user-info__img">
                                    <img width="40px" height="40px" src="'.$mention->profileImage.'" alt="'.$mention->firstName.' '.$mention->lastName.'">
                                </div>
